...
class HomographyBasisTransform(nn.Module):
    ...
    def __init__(
        ...
        self.basis_weights = nn.Parameter(
            torch.zeros(int(8 * self.batch_size), dtype=torch.float32)
        ).reshape(self.batch_size, 8, 1, 1, 1)
        self.flow_bases = gen_flow_bases(
            self.height, self.width, self.homography_weights
        )[None]
